...
// import { useAuth } from '../contexts/AuthContext'; // useAuth를 사용하지 않는다면 이 줄을 주석 처리하거나 삭제합니다.
...
const EventRecommendationPage = () => {
  ...
  // 백엔드에서 행사 데이터 불러오기
  useEffect(() => {
    fetch('http://localhost:5000/events/event-recommendations') // 실제 백엔드 주소로 변경
      .then(res => res.json())
      .then(data => {
        // --- 핵심 수정 부분: 데이터 전처리 ---
        const transformedData = data.map(event => {
          let decodedName = event.name;
          let decodedLocation = event.location;
          let decodedProfitability = event.profitability;
          let decodedCategory = event.category; // category 필드 추가

          // 유니코드 이스케이프 시퀀스 디코딩
          try {
            decodedName = JSON.parse(`"${event.name}"`);
            decodedLocation = JSON.parse(`"${event.location}"`);
            // profitability는 문자열일 때만 파싱하도록 조건 추가 (숫자면 에러 방지)
            decodedProfitability = typeof event.profitability === 'string' && event.profitability.startsWith('\\u')
                                    ? JSON.parse(`"${event.profitability}"`)
                                    : event.profitability;
            // category도 디코딩 및 기본값 설정
            decodedCategory = typeof event.category === 'string' && event.category.startsWith('\\u')
                              ? JSON.parse(`"${event.category}"`)
                              : event.category;
          } catch (e) {
            console.error("Failed to decode unicode in event fields:", e, event);
            // 디코딩 실패 시 원본 값 사용
          }

          // 날짜 형식 변환: "YYYY-MM-DD~YYYY-MM-DD" -> "YYYY.MM.DD - YYYY.MM.DD" 또는 "YYYY.MM.DD"
          const dateParts = event.date.split('~');
          let formattedDate = '';
          if (dateParts.length === 2) {
            formattedDate = `${dateParts[0].replace(/-/g, '.') } - ${dateParts[1].replace(/-/g, '.')}`;
          } else if (dateParts.length === 1) {
            formattedDate = dateParts[0].replace(/-/g, '.');
          } else {
            formattedDate = event.date; // 예상치 못한 날짜 형식
          }
          ...
          return {
            ...
            date: formattedDate,
            profitability: decodedProfitability,
            category: decodedCategory // 변환된 category 추가
          };
        });

        setEvents(transformedData); // 전처리된 데이터로 상태 업데이트
        setFilteredEvents(transformedData); // 초기 필터링도 적용
      })
      .catch(err => {
        console.error('행사 데이터 불러오기 실패:', err);
        // 실패 시 임시 데이터로 대체 (옵션). 이 부분도 전처리된 형식과 일치시켜야 합니다.
        setEvents([
          {
            id: 1,
            name: '서울 푸드 페스티벌',
            date: '2025.05.15 - 2025.05.17', // 변환된 형식으로 유지
            location: '서울 강남구 테헤란로 123',
            description: '다양한 음식과 문화를 즐길 수 있는 축제',
            image: 'https://via.placeholder.com/300x400',
            profitability: '높음', // 직접 한글로 입력
            weather: '맑음, 기온 15-20도',
            isFavorite: false,
            expectedVisitors: '약 5,000명',
            category: '푸드 페스티벌' // 직접 한글로 입력
          },
          // ...추가 임시 데이터
        ]);
        setFilteredEvents([
  ...
  // 컴포넌트 마운트 시 모든 이벤트 표시 (events 상태가 변경될 때마다 필터링된 이벤트 업데이트)
  useEffect(() => {
    setFilteredEvents(events);
  }, [events]); // events 상태가 변경될 때마다 재실행
